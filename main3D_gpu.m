...
for p = 1:num_phases
    phase{p} = (phase_IND == p);            % phase{p}(i,j,k) = 1 if X(i,j,k) belongs to the p-th phase
end      
...
Green = gpuArray(xi(:,:,:,IND_G(:,1)).*xi(:,:,:,IND_G(:,2))./sum(xi.^2, 4));	% Green operator
Green(isnan(Green)) = 0;                    % modify elements that are divided by zeros
%% SOLVE CELL PROBLEM
F = gpuArray(permute(repmat(F_Macro, 1, 1, n_nodes), [3, 1, 2]));  % initialize gradient deformation
P = gpuArray(zeros(n_nodes,ndim,ndim));                            % 1st Piola-Kirchhoff stress
K4 = gpuArray(zeros(n_nodes,ndim*ndim*(ndim*ndim+1)/2));           % tangent moduli
tic
for iter = 1:max_iter
    for p = 1 : num_phases
...
tic
S = gpuArray(zeros(n_nodes, ndim, ndim, ndim, ndim));       % fluctuation sensitivities
...
dK = gpuArray(zeros([n, ndim, ndim]));       % tangent increment
for i = 1:length(IND)
    dK(:,:,:,i) = fftn( sum(K4(:,:,:, IND(i,:)) .* dF(:,:,:, :), 4)); 
end
v = gpuArray(zeros([n, ndim, ndim]));
for i = 1:ndim
